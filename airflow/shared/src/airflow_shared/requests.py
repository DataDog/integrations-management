...
"""Minimal stdlib-only HTTP helper for talking to the Datadog API.

Mirrors gcp_shared/requests.py and azure_shared/common/requests.py: no
third-party HTTP library, so the built .pyz has no pip dependency for this
piece. Not yet wired to a real workflow-status endpoint (see reporter.py) --
kept here so that wiring one in later is a small, isolated change.
"""
...
def dd_request(method: str, path: str, body: Optional[dict[str, Any]] = None) -> tuple[str, int]:
    """Submit a request to the Datadog API, authenticated from DD_API_KEY/DD_APP_KEY/DD_SITE."""
    return request(
        method,
