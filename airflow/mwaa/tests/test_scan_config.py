...
def test_parse_scan_config_defaults_dd_site():
    config = parse_scan_config(["--session-id", SESSION_ID, "--region", "us-east-1"] + DD_API_KEY_ARGS)
...
def test_parse_scan_config_interactive_and_dry_run_flags():
    config = parse_scan_config(
        ["--session-id", SESSION_ID, "--region", "us-east-1", "--interactive", "--dry-run"] + DD_API_KEY_ARGS
    )
...
def test_parse_scan_config_raises_when_dd_api_key_missing(monkeypatch):
    ...
    with pytest.raises(ConfigError, match="Datadog API key is required"):
        parse_scan_config(["--session-id", SESSION_ID, "--region", "us-east-1"])
