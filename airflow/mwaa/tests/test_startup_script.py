...
from mwaa.startup_script import (
    ...
    startup_script_looks_configured,
)


def test_render_startup_script_includes_config_path_workaround_for_2_8_1():
    script = render_startup_script("2.8.1", "datadoghq.com", "my-mwaa-prod")
...
def test_render_startup_script_omits_workaround_for_newer_versions():
    script = render_startup_script("2.10.1", "datadoghq.com", "my-mwaa-prod")
...
def test_render_startup_script_never_carries_a_real_api_key():
    script = render_startup_script("2.10.1", "datadoghq.com", "my-mwaa-prod")
    assert f"export OPENLINEAGE_API_KEY={DD_API_KEY_PLACEHOLDER}" in script


def test_interpolate_api_key_substitutes_the_placeholder():
    script = render_startup_script("2.10.1", "datadoghq.com", "my-mwaa-prod")
    real = interpolate_api_key(script, "real-dd-api-key")
    assert "export OPENLINEAGE_API_KEY=real-dd-api-key" in real
