...
class WorkflowReporter:
    ...
    def report(
        ...
        """Report the status of a step in a workflow to Datadog."""
        response, response_status = dd_request_2(
            "POST",
            self.url,
            {
                "data": {
    ...
    def report_step(self, step_id: str, loading_message: str | None = None) -> Generator[StepStatusReporter, str, ]:
        ...
        self.report(step_id, Status2.IN_PROGRESS)
        try:
            yield StepStatusReporter(self, step_id)
        except Exception as e:
            self.report(step_id, Status2.FAILED, message=str(e))
            raise
        else:
            self.report(step_id, Status2.FINISHED)
